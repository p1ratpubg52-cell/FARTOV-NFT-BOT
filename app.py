...
# Telegram ID аккаунта @fart2_backpack
...
def normalize_owned_gift_for_catalog(
    owned
):
...
def distribute_gifts_between_cases(
    items
):
    ...
    for case_index, config in enumerate(
        CASE_CONFIGS
    ):
...
async def cases_catalog(
    ...
    cases = (
        distribute_gifts_between_cases(
            items
        )
    )
...
async def stars_invoice(
    ...
    link = await bot.create_invoice_link(
        ...
        description=
            f"Пополнение баланса на {amount} Stars",
